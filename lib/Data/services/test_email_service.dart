...
$userCompanyName Team''',
  ...
  static Future<String?> createTechPackPDF(List<String> imagePaths) async {
    try {
        ...
        if (imagePath.startsWith('/9j/') || imagePath.startsWith('iVBORw0KGgo') || imagePath.startsWith('data:image/')) {
          ...
          try {
            ...
            try {
              ...
              if (finalBytes.length > 12 * 1024) { // Slightly higher target for better quality
                finalBytes = await FlutterImageCompress.compressWithList(
                  Uint8List.fromList(bytes),
                  minHeight: 500,
                  minWidth: 500,
              ...
              if (kDebugMode) {
                print('🗜️ Compressed image for PDF ${(bytes.length / 1024).toStringAsFixed(1)} KB → ${(finalBytes.length / 1024).toStringAsFixed(1)} KB');
              }
            } catch (e) {
      ...
      final directory = await getTemporaryDirectory();
      final pdfPath = '${directory.path}/techpack_${DateTime.now().millisecondsSinceEpoch}.pdf';
      ...
      if (kDebugMode) {
        print('📄 Created PDF: $pdfPath (${(fileSize / 1024).toStringAsFixed(1)} KB)');
      }
      
      // If PDF is still too large, recreate with compressed images
      if (fileSize > 60 * 1024) { // 60KB limit (since 53.9KB worked fine)
        if (kDebugMode) {
        ...
        for (int i = 0; i < imagePaths.length && i < 3; i++) { // All 3 images for fallback
          ...
          if (imagePath.startsWith('/9j/') || imagePath.startsWith('iVBORw0KGgo') || imagePath.startsWith('data:image/')) {
            ...
            try {
              ...
              if (kDebugMode) {
                print('❌ Failed to process image ${i + 1} for fallback PDF: $e');
              }
            }
          }
        }
        ...
        if (kDebugMode) {
          print('📄 Created smaller PDF with 3 images: ${(smallPdfBytes.length / 1024).toStringAsFixed(1)} KB');
        }
          ...
          for (int i = 0; i < imagePaths.length && i < 3; i++) {
            ...
            if (imagePath.startsWith('/9j/') || imagePath.startsWith('iVBORw0KGgo') || imagePath.startsWith('data:image/')) {
              ...
              try {
                ...
                final tinyCompressed = await FlutterImageCompress.compressWithList(
                  Uint8List.fromList(bytes),
                ...
                if (tinyBase64.length < 8 * 1024) { // 8KB limit per template image
                  ...
                  if (kDebugMode) {
                    print('📎 Added tiny template image ${i + 1}: ${(tinyCompressed.length / 1024).toStringAsFixed(1)} KB');
                  }
                }
              } catch (e) {
                if (kDebugMode) {
  ...
  static Future<Uint8List?> compressImage(String imagePath) async {
    try {
      ...
      final targetSize = 15 * 1024; // Target 15KB per image for 3 images under 50KB total
      ...
      try {
        ...
        if (kDebugMode) {
          print('FlutterImageCompress result: ${(compressedBytes.length / 1024).toStringAsFixed(1)} KB');
        }
        ...
        if (kDebugMode) {
          print('Very compressed result: ${(veryCompressed.length / 1024).toStringAsFixed(1)} KB');
        }
        ...
        if (originalSize > targetSize) {
          ...
          if (kDebugMode) {
            print('Fallback truncation: ${(smallerBytes.length / 1024).toStringAsFixed(1)} KB');
          }
          return smallerBytes;
        }
      }
  ...
  static Future<Map<String, dynamic>> previewEmailWithPDF({
    ...
    try {
      ...
      final emailSubject = aiEmailData['subject']?.toString() ?? 'Production Inquiry from $userCompanyName';
      final emailBody = aiEmailData['body']?.toString() ?? 'Email content generation failed';
...
$userCompanyName Team''';
      ...
      try {
        ...
        if (emailBody.contains('[object Object]') || emailBody == 'null' || emailBody.isEmpty) {
          if (kDebugMode) {
...
$userCompanyName Team''';
      ...
      request.files.add(http.MultipartFile.fromBytes(
        'techpack_pdf',
        pdfBytes,
        filename: 'Atelia_TechPack_${DateTime.now().millisecondsSinceEpoch}.pdf',
      ));
...
$userCompanyName Team''';
      ...
      request.fields['template_params[time]'] = DateTime.now().toString().split('.')[0];
      request.fields['template_params[message]'] = cleanTemplateVariable(emailBody);
      ...
      if (kDebugMode) {
        print('🚀 Sending AI-powered email with REAL PDF attachment (${(pdfBytes.length / 1024).toStringAsFixed(1)} KB)');
        print('📧 This requires EmailJS Professional plan for file attachments');
      }
  ...
  static Future<bool> testEmailJSWithPDFAttachment({
    ...
    try {
      ...
      request.fields['to_email'] = toEmail; // EmailJS expects this at root level
      
      // Add template parameters with proper prefix for multipart form
      request.fields['template_params[name]'] = 'Atelia Fashion App';
      request.fields['template_params[time]'] = DateTime.now().toString().split('.')[0];
      request.fields['template_params[message]'] = cleanTemplateVariable(userMessage ?? 'Please find attached our complete tech pack PDF with all design images and specifications.');
      request.fields['template_params[email]'] = toEmail;
      request.fields['template_params[to_email]'] = toEmail; // Also in template for display
      request.fields['template_params[subject]'] = 'Tech Pack from Atelia Fashion App';
      ...
      try {
        for (int i = 0; i < imagePaths.length && i < 2; i++) { // Only 2 to save space
          final imagePath = imagePaths[i];
          if (imagePath.startsWith('/9j/') || imagePath.startsWith('iVBORw0KGgo') || imagePath.startsWith('data:image/')) {
            String cleanBase64 = imagePath;
            if (imagePath.startsWith('data:image/')) {
            ...
            try {
              ...
              final miniCompressed = await FlutterImageCompress.compressWithList(
                Uint8List.fromList(bytes),
              ...
              if (miniBase64.length < 5 * 1024) { // 5KB limit per template image
                request.fields['preview_image_${i + 1}'] = miniBase64;
                if (kDebugMode) {
                  print('📎 Added template preview ${i + 1}: ${(miniCompressed.length / 1024).toStringAsFixed(1)} KB');
                }
              }
            } catch (e) {
              if (kDebugMode) {
      ...
      if (kDebugMode) {
        print('🚀 Sending EmailJS form with PDF attachment (${(pdfBytes.length / 1024).toStringAsFixed(1)} KB)');
      }
