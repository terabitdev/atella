...
static Future<String?> createTechPackPDF(
  ...
  try {
        ...
        build: (pw.Context context) {
          return pw.Column(
            ...
            children: [
              ...
              // ✅ Production Requirements (ONLY place for Quantity, Cost, Delivery)
              pw.Container(
                width: double.infinity,
                padding: pw.EdgeInsets.all(15),
                decoration: pw.BoxDecoration(
    ...
    for (int i = 0; i < imagePaths.length && i < 3; i++) {
      ...
      if (imagePath.startsWith('/9j/') ||
        ...
        try {
          ...
          Uint8List finalBytes = await FlutterImageCompress.compressWithList(
    ...
    // --- FALLBACK COMPRESSION LOGIC (unchanged except border removed in PRODUCTION REQUIREMENTS) ---
    if (fileSize > 60 * 1024) {
          ...
          build: (pw.Context context) {
            return pw.Column(
              ...
              children: [
                ...
                // ✅ Production Requirements (no border)
                pw.Container(
                  width: double.infinity,
                  padding: pw.EdgeInsets.all(15),
                  decoration: pw.BoxDecoration(
      ...
      for (int i = 0; i < imagePaths.length && i < 3; i++) {
        ...
        if (imagePath.startsWith('/9j/') ||
          ...
          try {
            ...
            if (kDebugMode) {
              print('❌ Failed to process fallback image: $e');
            }
          }
        }
      }
...
$userCompanyName Team''';
      ...
      request.fields['reply_to'] = userEmail ?? '${FirebaseAuth.instance.currentUser?.email}';
