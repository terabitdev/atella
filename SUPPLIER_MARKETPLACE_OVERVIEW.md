...
- **Firebase Hosting** (`atelia-123.web.app`, free default domain, no new domain purchase) serves:
  - `public/.well-known/assetlinks.json` — Android Digital Asset Links, `com.company.atella` + SHA-256 cert fingerprints. JSON doesn't support comments, so the four entries are labeled here instead (in array order):
    1. `C1:4B:51:...:0E:55` — **confirmed release-signing certificate** (verified via `apksigner verify --print-certs` against a locally built `flutter build apk --release` output — the APK's actual signer matches this exactly, owner "Samama Hussain, TeraBit IT"). This is what a release APK built with this project's `key.properties`/keystore is actually signed with. Was accidentally dropped from this file at one point (an earlier edit reduced the list to only 2 entries), which broke App Links for any locally-built release APK until it was re-added.
    2. `2A:4C:86:...:5F:F2` — the other original entry (whoever first set up this feature); not yet confirmed which build it corresponds to — could be a debug fingerprint. Verify the same way (`apksigner verify --print-certs <apk>`) against whatever build it's meant to represent before relying on it.
